...
describe('PkgDetail test', () => {
    ...
    test('点击"查看"链接时正确触发showMulPackage改变', async () => {
        ...
        // 模拟点击"查看"链接
        const viewMulPackageLink = wrapper.find('.desc-link .el-link');
        await viewMulPackageLink.trigger('click');
        // 验证showMulPackage事件是否被正确触发
    ...
    test('点击"查看"链接时正确触发showCirleDep改变', async () => {
        ...
        // 模拟点击"查看"链接
        const viewMulPackageLink = wrapper.findAll('.desc-link .el-link')[1];
        await viewMulPackageLink.trigger('click');
        // 验证showMulPackage事件是否被正确触发
    ...
    test('点击"图像"链接时正确触发hilightCirleLinks事件', async () => {
        ...
        // 模拟点击"图像"链接
        const hightCirleLinksLink = wrapper.findAll('.desc-link')[1].findAll('.el-link')[1];
        console.log(hightCirleLinksLink.text());
